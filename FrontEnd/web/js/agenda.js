...
function cell(col, hora, iteracionDeSemana, horasString, ocupadas, iteracionDeHora) {// con la iteración puedo saber la hora
    ...
    if (hora && !ocupadas[iteracionDeHora]) { // true -> Botón habilitado
        ...
        tr.find("#atenderBtn").on("click", () => {
            ...
            // Pedir cita a DB y cargar en LocalStorage

            let cita = {hora: "", dia: ""};
            cita.hora = horasString[iteracionDeHora];
            cita.dia = arrayDiasSemana[iteracionDeSemana];
            console.log("ATENDER CITA!!  ==> "+ cita)

            //- Cargar en LocalStorage al Paciente
            //- Cargar Perfil y Leer LocalStorage
            localStorage.setItem("atenderCita", JSON.stringify(cita));
    ...
    $(".col-main").each(function () {
        ...
        if (!fetchSemana[i].checked) { // false | gray box ( - )
            ...
            horas.forEach((h) => {
                ...
                cell($(this), h, i, horasString, ocupadas, count);
                // ->     h  ==  1||0      i == 1,2,3,4,5 (l,m,i,j,v)
...
function validarFecha(weekCount, fecha) {
    ...
    console.log("mes: " + mes);
    document.getElementById("mesTitle").innerHTML = mes;
